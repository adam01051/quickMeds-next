...
const PharmacyDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [pharmacyId, setPropertyId] = useState<string | null>(null);
	const [property, setProperty] = useState<Property | null>(null);
	const [slideImage, setSlideImage] = useState<string>('');
	const [destinationProperties, setDestinationProperties] = useState<Property[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [pharmacyComments, setPropertyComments] = useState<Comment[]>([]);
	...
	const {
		loading: getPharmacyLoading,
		data: getPharmacyData,
		error: getPharmacyError,
		refetch: getPharmacyRefetch,
		...
		onCompleted: (data: T) => {
			if (data?.getPharmacy) setProperty(data.getPharmacy);
			if (data?.getPharmacy) setSlideImage(data?.getPharmacy?.pharmacyImages[0]);
		},
	});
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: property?.pharmacyLocation ? [property?.pharmacyLocation] : [],
				},
			},
		},
		skip: !pharmacyId || !property,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getPharmacies?.list) setDestinationProperties(data?.getPharmacies?.list);
		},
	});
	...
	const likePropertyHandler = async (user: T, id: string) => {
		try {
			...
			await getPharmacyRefetch({ input: id });
			await getPharmaciesRefetch({
				input: {
					page: 1,
					limit: 4,
					sort: 'createdAt',
					direction: Direction.DESC,
					search: {
						locationList: [property?.pharmacyLocation],
					},
				},
			});
	...
	if (device === 'mobile') {
		return <div>PHARMACY DETAIL PAGE</div>;
	} else {
		return (
						...
						<Stack className={'property-info-config'}>
										...
										<Stack className={'buy-rent-box'}>
											{property?.acceptsInsurance && (
												<>
													...
													<Typography className={'buy-rent'}>Insurance</Typography>
												</>
											)}
											...
											{property?.hasDelivery && (
												<>
													...
													<Typography className={'buy-rent'}>delivery</Typography>
												</>
											)}
										</Stack>
										<Stack className={'divider'}></Stack>
								...
								<Stack className={'right-box'}>
									<Stack className="buttons">
										<Stack className="button-box">
											...
											<Typography>{property?.pharmacyViews}</Typography>
										</Stack>
										<Stack className="button-box">
											...
											<Typography>{property?.pharmacyLikes}</Typography>
										</Stack>
									</Stack>
							...
							<Stack className={'images'}>
								...
								<Stack className={'sub-images'}>
									{property?.pharmacyImages.map((subImg: string) => {
						...
						<Stack className={'property-desc-config'}>
							<Stack className={'left-config'}>
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{property?.pharmacyMedicationCount}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
											<img src={'/img/icons/securePayment.svg'} />
										</Stack>
										<Stack className={'option-includes'}>
											<Typography className={'title'}>Pharmacy type</Typography>
											<Typography className={'option-data'}>{property?.pharmacyType}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
								...
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										<Typography className={'title'}>Pharmacy description</Typography>
										<Typography className={'desc'}>{property?.pharmacyDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>Pharmacy details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Delivery fee</Typography>
													<Typography className={'data'}>Delivery: ${formatterStr(property?.pharmacyDeliveryFee)}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Medication count</Typography>
													<Typography className={'data'}>{property?.pharmacyMedicationCount} medications</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Pharmacy type</Typography>
											...
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Opened year</Typography>
													<Typography className={'data'}>{property?.openedAt ? moment(property.openedAt).format('YYYY') : 'Not provided'}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Pharmacy type</Typography>
													<Typography className={'data'}>{property?.pharmacyType}</Typography>
												...
												<Box component={'div'} className={'info'}>
													...
													<Typography className={'data'}>
														{property?.acceptsInsurance && 'Insurance'} {property?.hasDelivery && 'Delivery'}
													</Typography>
												</Box>
											</Stack>
										</Stack>
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{pharmacyComments?.map((comment: Comment) => {
							...
							<Stack className={'similar-properties-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Nearby pharmacies</Typography>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationProperties.map((property: Property) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={property.pharmacyName}>
													<PropertyBigCard
														property={property}
